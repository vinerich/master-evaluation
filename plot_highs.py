...
for env in args.env:  # noqa: C901
    ...
    plt.ylabel("# Highs (Δx > 20)", fontsize=14)
    results[env] = {}
    ...
    for algo in args.algos:
        for folder_idx, exp_folder in enumerate(args.exp_folders):
            ...
            for _, dir_ in enumerate(dirs):
                ...
                try:
                    mean_ = np.squeeze(log["results_highs"].mean(axis=1))
                except:
                    mean_ = log["results_highs"]
...
if not os.path.exists(f"{DIR}/md"):
    os.makedirs(f"{DIR}/md", exist_ok=True)
...
with open(f"{DIR}/md/{args.output}.md", "w") as f:
    writer.stream = f
    writer.write_table()
...
if args.output is not None:
    ...
    if not os.path.exists(f"{DIR}/pkl"):
        os.makedirs(f"{DIR}/pkl", exist_ok=True)

    with open(f"{DIR}/pkl/{args.output}.pkl", "wb") as file_handler:
...
plt.tight_layout()
plt.savefig(f"{DIR}/img/{args.output}.svg")
