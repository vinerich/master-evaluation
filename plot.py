...
if not os.path.exists("score/md"):
    os.makedirs("score/md", exist_ok=True)
...
with open(f"score/md/{args.output}.md", "w") as f:
    writer.stream = f
    writer.write_table()
...
if args.output is not None:
    print(f"Saving to pkl/{args.output}.pkl")

    if not os.path.exists("score/pkl"):
        os.makedirs("score/pkl", exist_ok=True)

    with open(f"score/pkl/{args.output}.pkl", "wb") as file_handler:
        pickle.dump(post_processed_results, file_handler)

if not os.path.exists("score/img"):
    os.makedirs("score/img", exist_ok=True)

plt.tight_layout()
plt.savefig(f"score/img/{args.output}.svg")
