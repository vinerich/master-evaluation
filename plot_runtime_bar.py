    ...
    for algo in args.algos:
        for folder_idx, exp_folder in enumerate(args.exp_folders):
            ...
            for _, dir_ in enumerate(dirs):
                ...
                try:
                    mean_ = np.squeeze(log["train_time"].mean(axis=1))
                except:
                    mean_ = log["train_time"]
            ...
            if args.min_timesteps > 0:
                ...
                for n_timesteps in merged_timesteps:
                    if n_timesteps[-1] >= args.min_timesteps:
                        min_ = min_(min_, len(n_timesteps))
            ...
            if len(merged_results) > 0:
                ...
                min_ = np.min(mean_per_eval, axis=-1)
                max_ = np.max(mean_per_eval, axis=-1)
                # std: error:
...
for i, env in enumerate(args.env, start=1):
    ...
    plt.bar(y_pos, Y)
    plt.xticks(y_pos, X, rotation=30)

if not os.path.exists("traintime/md"):
    os.makedirs("traintime/md", exist_ok=True)
...
with open(f"traintime/md/{args.output}.md", "w") as f:
    writer.stream = f
    writer.write_table()
...
if args.output is not None:
    print(f"Saving to traintime/pkl/{args.output}.pkl")

    if not os.path.exists("traintime/pkl"):
        os.makedirs("traintime/pkl", exist_ok=True)

    with open(f"traintime/pkl/{args.output}.pkl", "wb") as file_handler:
        pickle.dump(post_processed_results, file_handler)

if not os.path.exists("traintime/img"):
    os.makedirs("traintime/img", exist_ok=True)

plt.tight_layout()
plt.savefig(f"traintime/img/{args.output}_bar.svg")
